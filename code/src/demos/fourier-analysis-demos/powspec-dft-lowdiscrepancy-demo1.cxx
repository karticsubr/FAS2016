...
int main(int argc, char* argv[]){
    ...
    for(int trial = 1; trial <= numTrials; trial++){
        ...
        for(int i = 0; i < N; i++){
            double x = i / double(N);
            double y = radicalInverse(i,2);

            finalsamples[2*i+0] = x;
            finalsamples[2*i+1] = y;

            int col = width * (x + minDomainX) * (1.0/maxRangeX);
            int row = height * (y + minDomainY) * (1.0/maxRangeY);
        ...
        if(trial % stepSize == 0 || trial == 1){
            ...
            //##########################################################
            ss.str(std::string());
            ss << images << "power-discrete-" << accumulate <<"-" << feature << "-" << mode << "-" << samplingpattern << "-u" <<ucomponent << "-v" << vcomponent << "-n" << nsamples << "-" << s1 << ".png";
            ...
            ss.str(std::string());
            ss << datafiles << "radial-mean-discrete-"  << accumulate <<"-" << mode << "-" << samplingpattern << "-n" << nsamples << "-" << s1 << ".txt";
