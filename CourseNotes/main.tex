...
\noindent Copyright \copyright\ 2016 held by the owner/author(s)\\ % Copyright notice
% 
\noindent \textsc{SIGGRAPH '16 Courses, July 24-28, 2016, Anaheim, CA.}\\ % Publisher
% 
\noindent Permission to make digital or hard copies of part or all of this work for personal or classroom use is granted without fee provided that copies are not made or distributed for profit or commercial advantage and that copies bear this notice and the full citation on the first page. Copyrights for third-party components of this work must be honored. For all other uses, contact the Owner/Author.\\ 
% License information
% 
\noindent \textit{ACM 978-1-4503-4289-6/16/07.} \\ % Printing/edition date
%
\noindent \url{http://dx.doi.org/10.1145/2897826.2927356}\\ % URL
...
% Example slides for this topic are included (chapter~\ref{ch:exslides}).
...
%
%\input{Pictures/fig-powspec-radialmean-random}
%
...
%
\input{Pictures/fig-powspec-radialmean-stochastic}
%
...
% More generally, this procedure can be expressed mathematically by starting with the $b$-ary representation of the positive integer $i$:
% %
% \begin{align}
% i=\sum_{k=0}^{L-1}d_k(n)b^k,
% \end{align}
% %
% where $b$ is the base in which number $i$ is represented, $L$ is the total number of necessary digits, and $0 <= d_k(n) < b$, is the $k$-th digit in the $b$-ary expansion of $i$. We can then flip the digits of this $b$-ary representation about the decimal point to create numbers in the interval $[0,1)$:
% %
% \begin{align}
% \Phi_b(i)=\sum_{k=0}^{L-1}d_k(n)b^{-k-1}.
% \end{align}
% %

% The sequence of values can be enumerated by and $i$ and constructed using a deterministic method that uses relative prime numbers as the base for each dimension, where the value for each coordinate can be enumerated deterministically by the sample index $i$.
...
%
%
\input{Pictures/fig-powspec-radialmean-deterministic}
%
...
%
\input{Pictures/fig-powspec-radialmean-bluenoise}
%
...
Figures~\ref{fig:powspec-radialmean-stochastic},~\RefFigNum{powspec-radialmean-deterministic} and~\RefFigNum{powspec-radialmean-bluenoise} depict radially averaged periodograms of the various sampling strategies described in this chapter. These spectra reveal two important characteristics of estimators built using the corresponding sampling strategies. 
...
\section{Normalizing Periodograms}
Since periodograms contain essential information regarding the sample distirbution, it is important to find a way to represent different sampling periodograms on a same scale.
%\paragraph{ periodograms} 
This is one of the major issues which is normally encountered while comparing periodograms of different sampling patterns. Since there is no well defined metric that could allow a fair comparison betweeen various sampling periodograms, it is not possible to inspect sampling periodograms by simply looking at them. We, therefore, describe one way of normalization that could allow fair comparison between different periodograms. For reference, we use random sampling periodogram for which we have an analytical form given by: $\Norm{\Fourier{\Sampling}(\FreqVar)} = \Dirac(\omega) \Npts^2 + \Npts$, given the sampling function is defined as: $\Sampling(x)=\sum_{k=1}^{\Npts} (x - x_k)$.
%
%\begin{align}
%\label{eq:random}
%\PowerSpec{\Sampling}(\FreqVar) &= 
%\begin{dcases*}
%       \Npts^2  & $ \FreqVar = 0 $\\
%         \Npts & otherwise
%        \end{dcases*}
%\end{align}
%
If you divide the random sampling periodogram by $\Npts$ you would obtain: $\Dirac(\omega) \Npts + 1$, which implies that at frequencies other than the DC, the power would be $1$ irrespective of the number of samples. 
We can use this same approach for the other sampling periodograms. We compute the power (magnitude squared Fourier coefficients) by dividing the magnitude square coefficient values by $\Npts$,i.e $\Norm{\Fourier{\Sampling}(\FreqVar)} / \Npts$, for visualization  purposes only. As a result of this normalization, the periodograms would always converge towards the power value of $1$ with increasing frequency, irrespective of the number of samples. 

%    to make other sampling periodograms look alike, irrespective of the number of samples used $\Npts$. 

\paragraph{Normalizing radially averaged periodograms} 
Once we obtain the periodograms using above normalization, we can compute the radially averaged periodograms that would have power converging towards $1$ towards higher frequencies. 
However, with the increase in number of samples the low frequency region of the radially averaged periodograms would keep scaling towards high frequencies. % as shown in~\RefFig{}. 
For comparison sake, we would like to avoid this scaling without compromising the underlying information. For this purpose, we can further divide the \emph{radial frequency variable $\RadialFreq$} by $\frac{1}{\alpha \sqrt[\Dim]{\Npts}}$, where $\Dim$ represents the dimensions and $\alpha > 0$ is a parameter that you can tweak to bring different radially averaged periodograms to the same scale. For regular sampling patterns, $\alpha=1$, because the first Fourier peak (alias) is at a distance $\sqrt[\Dim]{\Npts}$ from the DC peak in the regular sampling periodogram. As a result of this normalization of the radial frequency variable $\RadialFreq$, the radial periodogram shows the first peak at $\RadialFreq=1$ for regular pattern~(\RefFig{powspec-radialmean-deterministic}). All averaged and radially averaged periodograms shown in this chapter are illustrated using the above normalization heuristics.
