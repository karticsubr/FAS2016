...
% 
\noindent \textsc{SIGGRAPH 2016 Courses, July 24-28, 2016, Anaheim, CA.}\\ % Publisher
% 
...
% In this chapter, we will present a suite of tests that may be used to compare the efficacy of sampling patterns when used in numerical integration. We will then tabulate this comparison, identifying the strengths and weaknesses of the different sampling strategies described in chapter~\ref{ch:stateoftheart}. Our test suite will encompass a variety of integrands of various dimensionalities as well as smoothness criteria. We will make our code available to facilitate the comparison of any sampling patterns that may be proposed in the future with methods in our chosen set of algorithms. 
 ...
 %
\begin{tcolorbox}
./build/eea \cdash{S} $\cdots$ \cdash{I} $\cdots$ \cdash{A} $\cdots$  \cdash{G} $\cdots$
\tcblower
Here, \\
\cdash{S} flag is used to describe sampler name and  parameters. \\
\cdash{I} flag is used to describe the integrand name and parameters \\
\cdash{A} flag is used to describe analyzer name and parameters \\
\cdash{G} flag is used to give the prefix that you would like to add to your output files.
\end{tcolorbox}
 %
...
%
\begin{tcolorbox}
./build/eea \cdash{S} \cdashs{stype} stratified \cdash{A} \cdashs{atype} pts \cdashs{nsamps} 1024 \cdashs{nreps} 1 \cdash{G} \cdashs{ofile} prefix
\tcblower
Here, \\
\cdashs{stype} to give the sampler type. \\
\cdashs{atype} to give the Analyzer type, \\
\cdashs{nsamps} to give the number of samples \\
\cdashs{nreps} gives the number of trials or realizations \\
\cdashs{ofile} to give a prefix to your output filename
\end{tcolorbox}
%
...
%
\begin{tcolorbox}
./build/eea \cdash{S} \cdashs{stype} stratified \cdash{A} \cdashs{atype} pts \cdashs{nsamps} 4096 \cdashs{nreps} 1 \cdashs{tstep}  2 \cdashs{wstep} 1 \cdash{G} \cdashs{ofile} prefix
\tcblower
Here, \\
\cdashs{stype}: to give the sampler type. \\
\cdashs{atype}: to give the Analyzer type, \\
\cdashs{nsamps}: to give the number of samples \\
\cdashs{nreps}: gives the number of trials or realizations \\
\cdashs{tstep}: intermediate output after that many steps (it could be fractional) \\
