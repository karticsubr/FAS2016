...
We can use this same approach for the other sampling periodograms. We compute the power (magnitude squared Fourier coefficients) by dividing the magnitude square coefficient values by $\Npts$,i.e $\Norm{\Sampling(\FreqVar)} / \Npts$, for visualization  purposes only. As a result of this normalization, the periodograms would always converge towards the power value of $1$ with increasing frequency, irrespective of the number of samples. 
...
For comparison sake, we would like to avoid this scaling without compromising the underlying information. For this purpose, we can further divide the \emph{radial frequency variable} by $\frac{1}{\alpha \sqrt[d]{\Npts}}$, where $\Dim$ represents the dimensions and $\alpha > 0$ is a parameter that you can tweak to bring different radially averaged periodograms to the same scale. For regular sampling patterns, $\alpha=1$, because the first Fourier peak (alias) is at a distance $\sqrt[\Dim]{\Npts}$ from the DC peak in the regular sampling periodogram. All averaged and radially averaged periodograms shown in this chapter uses the above normalization heuristics to visualize periodograms.
