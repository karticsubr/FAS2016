...
Numerical integration methods like Monte Carlo are very powerful in approximating the underlying  multi-dimensional integrand using stochastically generated samples, but the overall process is highly error prone. Based on the sampling strategy, the error depends on an intricate combination of three factors: the distribution of the sample locations; the weighting used to accumulate
sampled contributions; and the arrangement and structure of
samples. This error can be distinguished in the form of bias and variance.

\subsection{Bias and Variance}
Bias corresponds to the value by which the numerically integrated value deviates from the ground truth value. In the Fourier domain, bias can be represented as an expected value of the error~\cite{Subr:2013:FAS}. Another form of error is variance which is manifested in the form of noise during image synthesis. Variance represents the uncertainty in the primary Monte Carlo estimates. 
...
However, it is possible to obtain error in the form of variance only for an unbiased estimator. 
 One way to obtain an unbiased estimator is to ensure that the expected Fourier coefficients of sample 
 points are non-zero only at the DC peak~\cite{Subr:2013:FAS}. This can be achieved by making sure that  
 the complex coefficients cancel out each other at all frequencies except at DC. 
 Pilleboue and colleagues have shown that this can be achieved by homogenizing the samples, that is,    by randomly and uniformly shifting each realization of the samples in the underlying space. This process is called homogenization. In the point processes literature, homogenization is also known as \emph{stationary point processes} for which the average number of points per some unit of extent such as length, area, volume, or time, is constant depending on the underlying mathematical space.
