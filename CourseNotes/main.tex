...
To overcome the issue of generating jittered sample points in high dimensions, Cook et al.~\cite{Cook:1984:DRT} proposed to randomly combine jittered sample sets over subspaces of the full domain. For instance, to generate sample distributions for a nine dimensional space, we could separately partition four two-dimensional subspaces (e.g.\ pixel area, camera lens area, reflection direction, and shadow ray direction) as well as one one-dimensional subspace (time) and then combine the subspace dimensions at random. He called this technique uncorrelated jittering.
...
The Halton sequence is a generalization of the one-dimensional van der Corput sequence to arbitrary dimensions. It operates by using van der Corput sequences with relative prime numbers as the bases for each dimension. Using the example of base $2$ and $3$ above, for instance, the Halton sequence would create a 2D point set by pairing the $i$-th entry of each one-dimensional sequence as the two dimensions of the $i$-th point:  $\left\{(\nicefrac{1}{2},\nicefrac{1}{3}), (\nicefrac{1}{4},\nicefrac{2}{3}), (\nicefrac{3}{4}, \nicefrac{1}{9}), (\nicefrac{1}{8}, \nicefrac{4}{9}), \cdots\right\}$.
...
Once we obtain the periodograms using above normalization, we can compute the radially averaged periodograms that would have power converging towards $1$ towards higher frequencies. However, with the increase in number of samples the low frequency region of the radially averaged periodogram would keep scaling towards high frequencies as shown in~\RefFig{}. For comparison sake we would like to avoid this scaling without compromising the underlying information. For this purpose, we can further divide the radially averaged periodogram power by $\frac{1}{\alpha \sqrt[d]{\Npts}}$, where $\Dim$ represents the dimensions and $\alpha > 0$ is a parameter that you can tweak to bring different radial periodograms to the same scale. For regular sampling patterns, $\alpha=1$.
...
\input{Pictures/fig-bluenoise-powspec-radialmean}
%
